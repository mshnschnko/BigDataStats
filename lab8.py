...
models = {
    "Гребневая регрессия": linear_model.Ridge,
    "Лассо": linear_model.Lasso
}
...
for i, alfa in enumerate(alfas):
    for j, (model_name, model_class) in enumerate(models.items()):
        ...
        axs[j, i].plot(x, y, "ro", markersize=2)
...
for j, (model_name, model_class) in enumerate(models.items()):
    ...
    axs[j].plot(x_test, model.predict(x_test))
    axs[j].plot(x, y, "ro", markersize=2)
